...
def read_mat(mat_path,cache_dir='/tmp'):
    ...
    if not os.path.isfile(cache_file):
        ...
        pickle.dump([image_paths, face_locations, genders, ages], open(cache_file, 'wb'))
    else:
        print "read from cache_file"
        image_paths, face_locations, genders, ages = pickle.load(open(cache_file, 'rb'))
    print "read mat OK"
    return image_paths, face_locations, genders, ages
...
def generate_caffe_txt_age(mat_path,output_path,cache_dir='/tmp',test_ratio=0.2,age_interval=10):
    ...
    image_paths, face_locations, genders, ages = read_mat(mat_path,cache_dir)
    # generate classes
